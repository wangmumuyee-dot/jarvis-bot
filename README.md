...
- 外币识别：可记录 USD、HKD、JPY、EUR、CNY，暂不做汇率换算
- 信用卡账单管理：设置和查询账单日、还款日
- 欠款管理：记录“我借出/我借入”，并按还款抵扣未结清欠款
- 财务周期：设置财务月起始日和每周起始日
- 分类统计：按本财务月统计 CNY 支出分类
- 账单日历：按天汇总本财务月 CNY 收入和支出
...
- 愿望储蓄：记录目标金额、已存金额和完成进度
- 快捷模板：保存常用指令，并通过“使用模板”自动执行
- 自动账单导入：扫描 `EXPORT_DIR/imports/*.xlsx`，导入后移动到 `imports/imported/`
- Obsidian 财务月报：写入 `02_Areas/财务`，可跟随 Obsidian Git 同步
- 消费分析：基于本月账单、预算和分类支出生成建议；配置 LLM 后会走大模型，否则使用本地规则 fallback
- 脱敏导出：导出 Excel 时可对身份证、银行卡、密码、key 等文本做脱敏
- 多设备同步检查：通过 Obsidian Git 写入同步检查文件并记录同步事件
